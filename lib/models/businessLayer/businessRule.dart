...
class BusinessRule {
  ...
  Future<bool> checkConnectivity() async {
    ...
    try {
      ...
      if (_connectivity == ConnectivityResult.mobile) {
        ...
        isConnected = true;
      } else if (_connectivity == ConnectivityResult.ethernet) {
        isConnected = true;
      } else {
        isConnected = false;
      }
      ...
      print('Exception - businessRule.dart - checkConnectivity(): $e');
    }
    return false;
  }
  ...
  Future<CroppedFile?> _cropImage(String sourcePath) async {
    try {
      CroppedFile? _croppedFile = await ImageCropper().cropImage(sourcePath: sourcePath, aspectRatio: CropAspectRatio(ratioX: 1, ratioY: 1), uiSettings: [
        AndroidUiSettings(
